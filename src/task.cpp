...
namespace RTSim {
  ...
  void Task::setKernel(AbsKernel *k) throw(KernAlreadySet)
    ...
    void Task::onArrival(Event *e)
    {
        ...
        if (toString().find("_21") != std::string::npos) 
        { cout << ""; }
    ...
    // Tick Task::getRemainingWCETCycles(MultiCoresScheds* queues) const {
    //     Task* _this = const_cast<Task*>(this);
    //     Tick remWCET = getWCET() - Tick::ceil(queues->getAlreadyExecutedCycles(_this, true));
    //     return remWCET;
    // }
    ...
    void Task::updateEndEvent() {
	   (*actInstr)->updateEndEvent();
    }
