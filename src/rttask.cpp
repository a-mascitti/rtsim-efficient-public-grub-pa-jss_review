...
namespace RTSim {
    ...
    void PeriodicTask::skipInstance() {
        arrEvt.drop();
        deadEvt.drop();
        deschedEvt.drop();
        endEvt.drop();
     ...
     */
    string PeriodicTask::toString() const {
        stringstream ss;
        // abs WCET = WCET without considering whether task CPU is clocked down/up
        ss << "PeriodicTask " << getName() << " WCET(abs) " << getWCET() << " DL = T " << getPeriod();
        return ss.str();
    }

    string NonPeriodicTask::toString() const {
        return "Non-" + PeriodicTask::toString();
    }
